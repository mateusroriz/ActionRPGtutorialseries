...
func _physics_process(delta):
	...
	match state:
		...
		CHASE:
			...
			if player != null:
				var direction = (player.global_position - global_position).normalized() #getting the difference in position between player and enemy
				velocity = velocity.move_toward(direction * MAX_SPEED, ACCELERATION * delta)
			else:
				state = IDLE #if player is not on range go back to idle
